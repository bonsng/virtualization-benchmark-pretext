...
import { MESSAGE_SEEDS } from '@shared/data'
...
const BASE_MESSAGE_SPECS: readonly MarkdownChatSeed[] = MESSAGE_SEEDS
...
export function buildConversationFrame(
  ...
  chatWidth: number,
  occlusionBannerHeight: number = 0,
  messageCount: number = TOTAL_MESSAGE_COUNT,
): ConversationFrame {
  ...
  for (let ordinal = 0; ordinal < messageCount; ordinal++) {
    ...
    const frameWidth = userFrameWidth
...
function layoutTemplateFrame(
  ...
  const frameWidth = Math.min(maxFrameWidth, contentInsetX * 2 + Math.max(1, usedContentWidth))
  return {
    blocks,
    bubbleHeight,
    contentInsetX,
